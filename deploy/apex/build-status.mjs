...
// Generated by ./scripts/funding-plan.mjs — never edit by hand. The previous
// version of the funding table below was written out longhand and covered nine
// EVM chains, so it silently understated the real cost by ~$140 (all of it
// Solana's program rent) and omitted every non-EVM chain.
const FUNDING = existsSync('deploy/apex/funding-plan.json')
  ? JSON.parse(readFileSync('deploy/apex/funding-plan.json', 'utf8'))
  : { rows: [] }
...
// Deploy cost per chain, from the generated funding plan rather than a
// hand-maintained literal. The literal it replaced listed ten EVM chains and
// rendered the word "non-EVM" for the other 24 — including Solana, whose $129
// of program rent is 85% of the entire deployment budget and was therefore
// invisible on the one page meant to show what deployment costs.
const COST = Object.fromEntries(
  FUNDING.rows.filter(r => r.usd != null).map(r => [r.key, r.usd]))
...
<div class="stats">
  ...
  <div class="stat"><b>$${FUNDING.rows.reduce((s, r) => s + (r.usd ?? 0), 0).toFixed(0)}</b><span>Cost, all ${FUNDING.rows.length} mainnets</span></div>
</div>
...
<tr><td><strong>Deployer wallet funded</strong></td><td class="no">NO</td><td>All on-chain deployment (~$${
  FUNDING.rows.reduce((s, r) => s + (r.usd ?? 0), 0).toFixed(0)} across ${FUNDING.rows.length} mainnets; see funding table)</td><td>Seb</td></tr>
...
<h2>How to fund — every mainnet, from Binance where possible</h2>
<div class="card pad" style="margin-bottom:18px">
  <p style="margin-bottom:12px">Generated by <code>node scripts/funding-plan.mjs</code>: live
  <code>eth_gasPrice</code> for EVM, and for non-EVM the amounts <strong>observed during the real
  testnet deployments</strong> — Solana's is the program rent that closing it actually refunded,
  Radix's is the fee the stokenet publish actually paid.</p>
  <p style="margin-bottom:12px">Pick the <strong>network</strong> column exactly. Sending on the
  wrong network loses the funds — and Binance makes this easy to get wrong: it reports
  <code>withdrawEnable: true</code> for RON and MNT on a <code>FIAT_MONEY</code> pseudo-network
  with no on-chain route at all. Headroom is 10x on gas-priced chains and 1.3x on rent-priced
  ones, because rent is a deterministic function of byte size and cannot spike.</p>
  <div class="scroll"><table style="min-width:820px">
  <thead><tr><th>Chain</th><th>Deploy cost</th><th>Withdraw</th><th>Route / network</th><th>Note</th></tr></thead>
  <tbody>
  ${(() => {
    const HEADROOM = { gas: 10, rent: 1.3 }
    const RENT = new Set(['solana','cardano','near','stellar','flow','algorand','sui'])
    const groups = [
      ['Fundable directly from Binance', r => r.route?.binance],
      ['Binance has no on-chain route', r => r.route?.alt],
      ['Needs a bridge', r => r.route?.bridge],
      ['No funding needed', r => r.route?.free],
    ]
    return groups.map(([title, pred]) => {
      const g = FUNDING.rows.filter(pred).sort((a, b) => (b.usd ?? -1) - (a.usd ?? -1))
      if (!g.length) return ''
      return `<tr><td colspan="5" style="background:#f7f9fb;font-weight:700;font-size:11px;
        text-transform:uppercase;letter-spacing:.06em;color:var(--dim)">${esc(title)} (${g.length})</td></tr>` +
        g.map(r => {
          const hr = (r.family !== 'evm' && RENT.has(r.key)) ? HEADROOM.rent : HEADROOM.gas
          const fund = r.native != null ? r.native * hr : null
          const route = r.route?.binance ? `Binance → <code>${esc(r.route.binance)}</code>`
            : r.route?.alt ? esc(r.route.alt)
            : r.route?.bridge ? esc(r.route.bridge)
            : r.route?.free ? esc(r.route.free) : '—'
          return `<tr><td><strong>${esc(r.key)}</strong></td>
            <td>${r.usd == null ? '—' : '$' + r.usd.toFixed(2)}</td>
            <td>${fund == null ? '—' : `<code>${fund < 0.001 ? '~0' : Number(fund.toPrecision(3))} ${esc(r.sym ?? '')}</code>`}</td>
            <td>${route}</td>
            <td class="wrap-ok">${esc(r.route?.why ?? r.route?.note ?? r.why ?? '')}</td></tr>`
        }).join('')
    }).join('')
  })()}
  </tbody></table></div>
  ${(() => {
    const tot = FUNDING.rows.reduce((s, r) => s + (r.usd ?? 0), 0)
    const HEADROOM = { gas: 10, rent: 1.3 }
    const RENT = new Set(['solana','cardano','near','stellar','flow','algorand','sui'])
    const fundTot = FUNDING.rows.reduce((s, r) =>
      s + (r.usd ?? 0) * ((r.family !== 'evm' && RENT.has(r.key)) ? HEADROOM.rent : HEADROOM.gas), 0)
    return `<p class="note"><strong>$${tot.toFixed(2)} to deploy all ${FUNDING.rows.length} mainnets;
    withdraw ~$${fundTot.toFixed(2)} to be safe.</strong> Solana alone is ${
      ((FUNDING.rows.find(r => r.key === 'solana')?.usd ?? 0) / (tot || 1) * 100).toFixed(0)
    }% of that — it is rent for a 245KB program, and it is <em>recoverable</em>: closing the
    program refunds it, which is exactly how the figure was measured.
    <br><br><strong>SKALE needs no funding and is still blocked</strong> — the chain is gasless, but
    contract deployment is gated behind a deployer whitelist. That is a request to the SKALE team,
    not money.
    <br><br>Prices as of ${esc(FUNDING.generated ?? '—')}. ${esc(FUNDING.note ?? '')}</p>`
  })()}
</div>
