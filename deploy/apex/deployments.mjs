...
export const DEPLOYMENTS = [
  ...
  {
    chain: 'EVM x18', network: 'MAINNET', family: 'evm', lang: 'Solidity',
    date: '2026-08-09',
    ...
    note:
      ...
      'injective, flow (EVM 747), flare, mantle, ronin, hedera, rootstock, beam, taiko. Verified per ' +
      ...
      'key-derived 0x addresses so the deployer already existed), Flare (listed with ' +
      'trading:false — no pair), Ronin and Mantle (FIAT_MONEY pseudo-network, no on-chain ' +
      'route) all came from bridging leftover Base gas via LI.FI for ~$1.80 total; ' +
      'Rootstock — the most expensive EVM deploy we have at $8, because RBTC is pegged to ' +
      'BTC — was paid for entirely by consolidating idle gas off Ethereum, Base, Arbitrum, ' +
      ...
      'Taiko came via LAYERSWAP, a fourth bridge family: LI.FI does not list chain 167000 ' +
      'at all, GasZip answers "Chain Limit Exceeded" for it at every size, and Squid does ' +
      'not carry it either — Layerswap moves ETH there with a $0.15 minimum, paid out of ' +
      'idle Scroll gas.',
    checks: [
      { name: 'bytecode present on all 18', result: 'PASS', detail: '12,890 bytes on every chain' },
      { name: 'owner is the retained key',  result: 'PASS', detail: '0xD10178e0…, required by Retro9000 / OP Atlas' },
      { name: 'revenue routed off the hot key', result: 'PASS', detail: 'treasuryReceiver = 0xB8156B85… on all 18' },
