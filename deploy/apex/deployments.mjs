...
export const BLOCKED_DEPLOYMENTS = [
  ...
  { chain: 'Radix', network: 'stokenet',
    reason: 'FUNDED (10,000 XRD, no wallet — the on-chain faucet component called directly with a notarised transaction) and the WASM NOW BUILDS: 579,736 bytes via plain `cargo build --target wasm32-unknown-unknown` with RUSTFLAGS="-C link-arg=--allow-undefined". The Radix Engine host functions (object_call, buffer_consume, sys_panic…) are wasm IMPORTS, and without that flag rust-lld rejects them as undefined symbols. What still blocks deployment is the package DEFINITION (.rpd): only `scrypto build` produces it, and scrypto strips RUSTFLAGS — verified against `-e RUSTFLAGS=…`, `--custom-option --config target.*.rustflags`, and a .cargo/config.toml, none of which reach its linker line. Pinning rustc 1.81 does not help either: scrypto cannot parse that toolchain\'s target info, and the dependency tree needs a newer manifest format. Needs an upstream scrypto fix or a wasm runtime to extract the schema export.' },
