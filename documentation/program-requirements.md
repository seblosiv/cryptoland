...
and extracted their real fields. **40 programmes, 126 pages, 7 real forms.**
Evidence: `scripts/formaudit/deep-2026-08-14.jsonl`.

Coverage is 40 of 64, not all — the residential proxy died mid-run and ~30
programmes came back as ~300-char `ERR_TUNNEL_CONNECTION_FAILED` pages. Those
were **discarded and re-crawled directly**, not reported: a page that never
loaded is *unknown*, not *empty*. The remaining 24 are mostly entries already
marked DEAD/BLOCKED.

### Four traps, one shape

Every one was recorded as a usable application by the shallow sweep. Only
rendering the form revealed otherwise:

| # | Programme | Looked like | Actually is |
|---|---|---|---|
| 25/27 | Polygon | "APPLY NOW" on the marketing page | Portal says **"Program Closed"**; S2 recipients already selected |
| 17 | MultiversX | An application Typeform | **"Developer Office Hours Request Form"** — and it redirects to `typeform.com/private-typeform`, so it is not publicly fillable at all |
| 41 | Solana Foundation | A grant form on the grants page | A **hiring form**: *"What positions are you hiring for?"*, *"Full Time or Contract roles?"* |
| 56 | Taiko | "Submit a project" | **Invite-only**: a required field asks *"Which Taiko Team Member(s) Did You Receive Your Invitation to Apply?"* |

§21 said *"structure never identifies intent — read the words."* This round
sharpens it: **a form-shaped URL on a grants page is usually not the grant
form.** Four of seven real forms found were something else entirely.

A fifth near-miss worth recording: The Graph (#59) has a **live** Typeform at
`/to/rhYddDRu` with `isFormClosed:false` — but it is the *"Inbound Partnership
Form"*, not grants. The grants form `/applynow` still has zero inputs. The DEAD
status stands; the partnership form is a genuine channel if we ever approach
them as a subgraph consumer rather than a grantee.

### The required fields, read from the live forms

| # | Programme | Engine | Inputs | Required |
|---|---|---|---|---|
| 31 | Arbitrum Foundation Grants | Airtable | 12 | 6 |
| 47 | Arbitrum Gaming Ventures | Airtable | 12 | 6 |
| 33 | SafePal Builder's Grant | Google Forms | 41 | 17 |
| 33 | SafePal (token listing) | Google Forms | 19 | 5 |
| 41 | Solana Foundation | Google Forms | 24 | 8 |
| 56 | Taiko Grants | Airtable | 16 | 5 |
| 59 | The Graph (partnerships) | Typeform | 2 | 2 |

**Both Arbitrum forms require the same three fields:** Project Name\*, Email\*,
**Telegram handle\***, **X/Twitter profile link\***, **Github link\***.

**SafePal requires traction we do not have:** **on-chain MAU\*** with **proof
link\***, **unique interacting wallets\*** with **proof link\***, **TVL\***,
audit reports, CMC/CoinGecko listing. Deadline on the page: **2026/12/31 23:59
UTC**.

### What this settles — the answer to "can we submit yet"

Three mandatory fields **do not exist**: a public GitHub URL, an X handle, a
Telegram handle. Three more are **zero**: on-chain MAU, unique interacting
wallets, TVL — `totalSupply()` is 0 on all 27 deployments and no tile has been
bought.

This is no longer inferred from documentation; it is scraped from the live
forms. **The blocker is not research.** Publishing the repo, creating two
accounts and making one real purchase converts six mandatory blanks into six
filled fields across Arbitrum ×2, Starknet, Polygon and SafePal simultaneously.
No further crawling changes that.

### Contact channels harvested (junk filtered)

`eco@safepal.com` · `Marketing@safepal.com` · `contact@celestia.org` ·
`devrel-am@animocaminds.com` · `bd@flowfoundation.org` · `info@thegraph.com` ·
`press@cardanofoundation.org` · `karl@optimism.io` · `support@alchemy.com`
Discord: arbitrum, sui, aztec, flow, rootstock, roninnetwork, oasysofficial,
graphprotocol, alchemy-builders.
Telegram: arbitrum, mantlenetwork, FlareNetwork, NEAR_HouseOfStake, SafePalTG,
rskofficialcommunity, CardanoAnnouncements.

Scraped personal Gmail addresses and GitHub noreply addresses were filtered out
— a stray address harvested from a page is not a programme's contact channel,
and listing it as one makes the table untrustworthy.

### Four bugs the crawler needed first

Recorded because each produced *plausible-looking but empty* output, which is
the dangerous failure mode — the run looks like it worked:

1. A **multi-line JS regex** inserted by a patch broke the entire form probe
   silently; every form returned zero questions.
2. **Strict host matching** rejected every link. Programmes routinely put apply
   and docs pages on a sibling subdomain (`developers.radixdlt.com` →
   `www.radixdlt.com`). Comparing registrable domain took `pages=1` → `pages=8`.
3. `dismiss()` looped nine cookie words at `timeout=1.5` each — **13 seconds per
   page** with no banner present, which reads as a hang.
4. Asset URLs (`.css`, `.js`) consumed page slots.

Plus one launch bug worth its own line: **inline multi-command SSH blocks were
dying partway and reporting success**, so target files were never created and
every "launch" read a path that did not exist. Write the launcher as a script
file that verifies its own inputs. And `pgrep -fc <pattern>` counts *your own
SSH command* — check `pgrep -f 'python3 /root/deep_0814.py' | wc -l` instead.

**Data-quality rule that came out of it:** a page is only a FORM if it has
inputs. The Questbook portal yielded nine "questions" that were Medium blog
headlines; 15 such link-lists were rejected across the run. Recording those as
findings is worse than recording nothing, because they look like data.

### Aggregator diff

Five candidates surfaced that the list had never carried. Each verified
individually: **IOTA DLT Foundation** (#61, OPEN — Tier-1 $10k with **KYC not
required**; IOTA EVM verified live at chainId **8822** with
`access-control-allow-origin: *`, so an EVM build target is ~2-4h) and
**Circle** (#62, OPEN but a payments pivot, not an integration) were added;
**Chainlink BUILD** (#63) and **Aleph Zero** (#64) were added already DEAD with
the quotes that killed them. **Moonbeam was a trap** — a stale directory entry
for a chain `config.js` already documents as wound down. Its head is now **4.3
days** frozen, up from 27.6 hours when first recorded.
