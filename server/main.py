...
# Imported as modules, not names: chain_pay reads env at call time (treasury
# addresses) and both are regenerated by scripts/, so a `from x import y` would
# pin stale values and hide regeneration behind a restart.
import chain_pay as _chain_pay
import chain_registry as _chain_registry
...
# Which chain THIS backend is. Set per instance by the systemd unit
# (CRYPTOLAND_CHAIN=base). The native purchase path prices and verifies against
# this chain only — a Base backend must never settle a Polygon transaction.
CHAIN_KEY = (os.environ.get("CRYPTOLAND_CHAIN") or "polygon").strip().lower()
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not users_cols:
            ...
            # telegram_id belongs HERE, not only in the migration branch below:
            # the unique index a few lines further down references it
            # unconditionally, so omitting it made init_db raise
            # "no such column: telegram_id" on every brand-new database and
            # took the whole backend down at startup. Existing deployments
            # never hit it because their DBs predate the index.
            await db.execute("""
                CREATE TABLE users (
        ...
        """)
        # ── Native-wallet purchase quotes ──────────────────────────────────
        # A quote is a price the server promised for one tile on one chain,
        # good until expires_at. It is what makes the native path safe: the
        # amount charged is decided here and re-checked against the chain, so
        # nothing the browser sends can change what a tile costs.
        await db.execute("""
        ...
        """)
        # One transaction settles at most one tile, forever. Without this a
        # single payment could be replayed against every quote the buyer opens.
        # Partial index so the many un-paid quotes (tx_hash NULL) don't collide.
        await db.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS idx_chain_quotes_txhash
            ON chain_quotes(chain, tx_hash) WHERE tx_hash IS NOT NULL
        """)
        await db.execute("""
            CREATE INDEX IF NOT EXISTS idx_chain_quotes_tile
            ON chain_quotes(tile_key, consumed_at)
        """)
...
# ── Native wallet purchases ──────────────────────────────────────────────────
#
# Pay for a tile with the chain's own token, from the user's own wallet: on Base
# you send ETH on Base. Three steps, and the server decides both of the ones
# that involve money.
#
#   POST /chain/quote    server prices the tile and locks that price in
#   (wallet)             user sends exactly that to the treasury
#   POST /chain/verify    server reads the chain and writes the tile
#
# The invariants here are the same ones §4 of CLAUDE.md requires of the
# NOWPayments rail, with one deliberate improvement: /np/payment stores
# `price_usd` from a CLIENT-SUPPLIED field, so a crafted request buys any tile
# for any amount. That is not repeated — build_quote() computes the price from
# tile_pricing.py and the browser is never asked.

class ChainQuoteRequest(BaseModel):
    tx:         int
    ty:         int
    tile_key:   str
    country:    Optional[str] = "Unknown"
    color:      Optional[str] = None
    payer:      Optional[str] = None   # wallet address, for display only
    ref_code:   Optional[str] = None
    session_id: Optional[str] = None
...
@app.get("/chain/pay-info")
async def chain_pay_info():
    """
    Can this build take a wallet payment, and in what token?

    The purchase UI calls this before offering the native path, so a chain with
    no treasury configured, no price feed or no verifier degrades to the
    off-chain rail instead of showing a button that fails.
    """
    chain = _chain_registry.get_chain(CHAIN_KEY)
    if chain is None:
        return {"enabled": False, "reason": f"Unknown chain: {CHAIN_KEY}"}
...
@app.post("/chain/quote")
@limiter.limit("20/minute")
async def chain_quote(req: ChainQuoteRequest, request: Request):
    """Price a tile for native payment and hold that price for 15 minutes."""
    if not (0 <= req.tx <= 16383 and 0 <= req.ty <= 16383):
        raise HTTPException(400, "Tile coordinates out of range")
    if req.tile_key != f"{req.tx}:{req.ty}":
        raise HTTPException(400, "tile_key does not match coordinates")

    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        user = await _require_auth(request, db)
        # Identity comes from the token. A client-supplied owner here would be
        # free tile claiming, exactly as it would be on /purchase.
        owner = user.get("wallet") or user.get("user_id")
        ...
        async with db.execute(
            ...
            existing = await cur.fetchone()
        if existing:
            raise HTTPException(409, "That tile already has an owner")

        async with db.execute("SELECT COUNT(*) FROM blocks") as cur:
            sold_count = (await cur.fetchone())[0] or 0

    # Market events move the price the map shows; charge what was shown.
    multiplier = 1.0
    try:
        ctx = await get_events_for_tile(req.tile_key, req.country or "Unknown")
        multiplier = float(compute_final_multiplier(ctx) or 1.0)
    except Exception as exc:
        print(f"[ChainPay] price-event multiplier unavailable, using 1.0: {exc}")

    try:
        quote = await _chain_pay.build_quote(
            tx=req.tx, ty=req.ty, chain_key=CHAIN_KEY, owner=owner,
            payer=req.payer, sold_count=sold_count, multiplier=multiplier,
        )
    except _chain_pay.ChainPayError as exc:
        raise HTTPException(400, str(exc))

    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            INSERT INTO chain_quotes (
                quote_id, tile_key, tx, ty, chain, owner, price_usd,
                native_amount, native_symbol, native_decimals, usd_per_token,
                treasury, payer, country, color, ref_code, session_id,
                created_at, expires_at
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            quote.quote_id, quote.tile_key, quote.tx, quote.ty, quote.chain_key,
            owner, quote.price_usd, str(quote.native_amount), quote.symbol,
            quote.decimals, quote.usd_per_token, quote.treasury, req.payer,
            req.country, req.color, req.ref_code, req.session_id,
            quote.created_at * 1000, quote.expires_at,
        ))
        await db.commit()

    print(f"[ChainPay] Quote {quote.quote_id[:8]} {quote.tile_key} "
          f"${quote.price_usd} = {quote.native_display} {quote.symbol}")
    return quote.to_public()

@app.post("/chain/verify")
@limiter.limit("30/minute")
async def chain_verify(req: ChainVerifyRequest, request: Request):
    """
    Confirm the buyer's transaction on-chain, then write the tile.

    Returns 202 with {"pending": true} while the transaction is still
    confirming — the client polls. Anything genuinely wrong (underpaid, wrong
    recipient, reverted) is a 4xx and will not become right by waiting.
    """
    tx_hash = (req.tx_hash or "").strip()
    if not tx_hash:
        raise HTTPException(400, "Missing transaction hash")

    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        user = await _require_auth(request, db)
        async with db.execute(
            "SELECT * FROM chain_quotes WHERE quote_id = ?", (req.quote_id,)
        ) as cur:
            q = await cur.fetchone()
        if not q:
            raise HTTPException(404, "Unknown quote")

        # The quote belongs to whoever asked for it. Otherwise one user could
        # settle another user's quote against their own tile.
        caller = user.get("wallet") or user.get("user_id")
        if str(q["owner"]).strip().lower() != str(caller).strip().lower():
            raise HTTPException(403, "That quote belongs to another account")
        if q["consumed_at"]:
            raise HTTPException(409, "This quote has already been settled")
        if int(time.time()) > int(q["expires_at"]):
            raise HTTPException(410, "Quote expired — take a new price")

        # Reject a hash already spent on any other quote before doing the RPC
        # work. The unique index is the real guarantee; this is the clean error.
        async with db.execute(
            "SELECT quote_id FROM chain_quotes WHERE chain = ? AND tx_hash = ? AND quote_id != ?",
            (q["chain"], tx_hash, req.quote_id),
        ) as cur:
            if await cur.fetchone():
                raise HTTPException(409, "That transaction has already been used for another tile")

    chain = _chain_registry.get_chain(q["chain"])
    if chain is None:
        raise HTTPException(400, f"Unknown chain: {q['chain']}")

    quoted = int(q["native_amount"])
    # Same 5% tolerance /np/finalize allows: wallets round, and some chains
    # take the fee out of the transferred amount.
    min_amount = int(quoted * _chain_pay.MIN_PAYMENT_RATIO)

    proof = await _chain_pay.verify_payment(chain, tx_hash, q["treasury"], min_amount)

    if not proof.ok:
        if proof.pending:
            return JSONResponse(
                status_code=202,
                content={"pending": True, "message": proof.error or "Confirming",
                         "confirmations": proof.confirmations,
                         "required": chain.confirmations},
            )
        raise HTTPException(402, proof.error or "Payment could not be verified")

    now_ms   = int(time.time() * 1000)
    tile_key = q["tile_key"]
    owner    = q["owner"]
    price    = float(q["price_usd"])   # server's number, never the client's

    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        try:
            # Re-check consumption inside the exclusive transaction: two tabs
            # can reach this point with the same quote at the same time.
            await db.execute("BEGIN EXCLUSIVE")
            async with db.execute(
                "SELECT consumed_at FROM chain_quotes WHERE quote_id = ?", (req.quote_id,)
            ) as cur:
                fresh = await cur.fetchone()
            if fresh and fresh["consumed_at"]:
                await db.execute("ROLLBACK")
                raise HTTPException(409, "This quote has already been settled")
            ...
            if taken and str(taken["owner"]).strip().lower() != str(owner).strip().lower():
                # Someone bought it between quote and settlement. The payment is
                # real and on-chain, so say so plainly rather than swallowing it.
                await db.execute("ROLLBACK")
                raise HTTPException(
                    409,
                    "That tile was claimed while your payment was confirming. "
                    "Your transaction is on-chain — contact support for a refund.",
                )
            ...
            await db.execute("""
                ...
                ON CONFLICT(tile_key) DO UPDATE SET
                    color        = excluded.color,
                    purchased_at = excluded.purchased_at
            """, (tile_key, q["tx"], q["ty"], owner, q["color"] or "#4ade80",
                  price, q["country"] or "Unknown", chain.name, now_ms))

            await db.execute(
                "UPDATE chain_quotes SET consumed_at = ?, tx_hash = ?, payer = ? WHERE quote_id = ?",
                (now_ms, tx_hash, proof.payer or q["payer"], req.quote_id),
            )
            await db.commit()
            ...
            await db.execute("ROLLBACK")
            raise HTTPException(500, f"Could not record the purchase: {exc}")

    print(f"[ChainPay] Settled {tile_key} → {owner} @ ${price} "
          f"({chain.symbol} tx {tx_hash[:14]}…)")

    if q["ref_code"]:
        await _process_referral_commission(
            ref_code=str(q["ref_code"]).upper().strip(),
            referee_wallet=_norm_wallet(owner) if str(owner).startswith("0x") else None,
    ...
    result["explorer_url"] = f"{chain.explorer_tx}{tx_hash}" if chain.explorer_tx else None
    result["paid_native"] = _chain_pay.from_base_units(proof.paid, chain.decimals)
