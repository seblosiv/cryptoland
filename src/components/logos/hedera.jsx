/**
 * Hedera — official brand mark, real path data (simple-icons), monochrome.
 * One path: the disc with the barred "H" knocked out by reversed sub-path
 * winding, so a currentColor fill shows the glyph rather than a solid disc.
 */
export default function HederaLogo({ size = 28, className, style }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
         xmlns="http://www.w3.org/2000/svg" className={className} style={style}
         role="img" aria-label="Hedera logo">
      <path
        fill="currentColor"
        d="M12 0a12 12 0 1 0 0 24 12 12 0 0 0 0-24Zm4.9571 17.3963h-1.5812V14.01H8.6224v3.3777H7.0498V6.6037H8.631v3.3845h6.7535V6.6037h1.5812zm-1.5812-6.2592H8.6224v1.7241h6.7535Z"
      />
    </svg>
  )
}
